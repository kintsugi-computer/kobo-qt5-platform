...
NiLuJe's FBInk in an APK package for installing into an Alpine chroot on a Kobo eReader.
...
For instance, from the command-line in an Alpine chroot:
    ...
    latest="https://api.github.com/repos/kintsugi-computer/kobo-fbink/releases/latest"
    release="$(wget -O - "${latest}" | /bin/sed -n 's/.*browser_download_url": "\(.*\)"/\1/p')"
    wget "${release}"

    apk add --allow-untrusted kobo-fbink*.apk

### Compiling and Packaging FBInk
...
* Switch to the newly created user 
    * Start a s shell as the new user `su - username` 
